...
(function($) {
    ...
    $.TokenList = function(input, url_or_data, settings) {
        ...
        if (
            typeof url_or_data === "string" ||
            typeof url_or_data === "function"
        ) {
        ...
        var input_box = $(
            ...
            .blur(function() {
                ...
                $(this).val("");
                token_list.removeClass(
                    $(input).data("settings").classes.focused
                );
            })
            ...
            .keydown(function(event) {
                ...
                switch (event.keyCode) {
                    ...
                    case KEY.COMMA:
                        if (selected_dropdown_item) {
                            add_token(
                                $(selected_dropdown_item).data("tokeninput")
                            );
                            hiddenInput.change();
                        } else {
                            if ($(input).data("settings").allowFreeTagging) {
                                if (
                                    $(input).data("settings").allowTabOut &&
                                    $(this).val() === ""
                                ) {
                                    return true;
                                } else {
        ...
        var token_list = $("<ul />")
            ...
            .mouseover(function(event) {
                ...
                if (li && selected_token !== this) {
                    li.addClass(
                        $(input).data("settings").classes.highlightedToken
                    );
                }
            })
        ...
        hiddenInput.val("");
        var li_data =
            $(input).data("settings").prePopulate || hiddenInput.data("pre");
        ...
        if (
            ...
            li_data = $(input)
                .data("settings")
                .onResult.call(hiddenInput, li_data);
        }
        ...
        function escapeHTML(text) {
            return $(input).data("settings").enableHTML
                ? text
                : _escapeHTML(text);
        }
        ...
        function toggleDisabled(disable) {
            if (typeof disable === "boolean") {
                ...
                $(input).data("settings").disabled = !$(input).data("settings")
                    .disabled;
            }
        ...
        function resize_input() {
            ...
            input_resizer.html(
                _escapeHTML(input_val) || _escapeHTML(settings.placeholder)
            );
        ...
        function add_freetagging_tokens() {
            ...
            $.each(tokens, function(i, token) {
                ...
                if ($.isFunction($(input).data("settings").onFreeTaggingAdd)) {
                    token = $(input)
                        .data("settings")
                        .onFreeTaggingAdd.call(hiddenInput, token);
                }
                var object = {};
        ...
        function insert_token(item) {
            ...
            if (readonly)
                $this_token.addClass(
                    $(input).data("settings").classes.tokenReadOnly
                );
        ...
        function add_token(item) {
            ...
            if (
                token_count > 0 &&
                $(input).data("settings").preventDuplicates
            ) {
                var found_existing_token = null;
                token_list.children().each(function() {
                    var existing_token = $(this);
                    var existing_data = $.data(
                        existing_token.get(0),
                        "tokeninput"
                    );
                    if (
                        existing_data &&
                        existing_data[settings.tokenValue] ===
                            item[settings.tokenValue]
                    ) {
        ...
        function populateDropdown(query, results) {
            ...
            if (results && results.length) {
                ...
                $.each(results, function(index, value) {
                    ...
                    if (index % 2) {
                        this_li.addClass(
                            $(input).data("settings").classes.dropdownItem
                        );
                    } else {
                        this_li.addClass(
                            $(input).data("settings").classes.dropdownItem2
                        );
                    }

                    if (
                        index === 0 &&
                        $(input).data("settings").autoSelectFirstResult
                    ) {
                ...
                if ($(input).data("settings").noResultsText) {
                    dropdown.html(
                        "<p>" +
                            escapeHTML(
                                $(input).data("settings").noResultsText
                            ) +
                            "</p>"
                    );
                    show_dropdown();
        ...
        function select_dropdown_item(item) {
            if (item) {
                ...
                item.addClass(
                    $(input).data("settings").classes.selectedDropdownItem
                );
            ...
            if (cached_results) {
                if ($.isFunction($(input).data("settings").onCachedResult)) {
                    cached_results = $(input)
                        .data("settings")
                        .onCachedResult.call(hiddenInput, cached_results);
                }
                ...
                if ($(input).data("settings").url) {
                    ...
                    // Prepare the request
                    ajax_params.data[
                        $(input).data("settings").queryParam
                    ] = query;
                    ajax_params.type = $(input).data("settings").method;
                    ajax_params.dataType = $(input).data(
                        "settings"
                    ).contentType;
                    ...
                    if ($(input).data("settings").excludeCurrent) {
                        ...
                        var tokenList = $.map(currentTokens, function(el) {
                            if (
                                typeof $(input).data("settings").tokenValue ==
                                "function"
                            )
                                return $(input)
                                    .data("settings")
                        ...
                        ajax_params.data[
                            $(input).data("settings").excludeCurrentParameter
                        ] = tokenList.join(
                            $(input).data("settings").tokenDelimiter
                        );
                    }
                    ...
                    ajax_params.success = function(results) {
                        ...
                        if ($.isFunction($(input).data("settings").onResult)) {
                            results = $(input)
                                .data("settings")
                                .onResult.call(hiddenInput, results);
                        }
        ...
        function computeURL() {
            ...
            return typeof settings.url == "function"
                ? settings.url.call(settings)
                : settings.url;
        }
